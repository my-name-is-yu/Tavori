        ...
        <>
          {!data.exists && (
            <p className="text-xs mb-3 px-3 py-2 rounded" style={{ color: 'var(--status-warning)', background: 'color-mix(in srgb, var(--status-warning) 10%, transparent)', border: '1px solid color-mix(in srgb, var(--status-warning) 25%, transparent)' }}>
              ~/.motiva/provider.json not found — showing defaults
            </p>
          )}
    ...
    <SectionCard title="Plugins">
      {error ? (
        ...
        <p className="text-sm" style={{ color: 'var(--text-tertiary)' }}>
          No plugins found in ~/.motiva/plugins/
        </p>
      ) : (
...
function SystemHealthSection() {
  const connected = useMotivaStore((s) => s.connected);
  ...
  return (
    <SectionCard title="System Health">
      ...
      <FieldRow label="Motiva Version" value="0.1.0" />
      <FieldRow label="Data Directory" value="~/.motiva/" />
      <FieldRow
