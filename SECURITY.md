...
Thank you for helping keep Conatus secure. This document describes how to report security vulnerabilities and explains Conatus's security architecture.

## Reporting Security Vulnerabilities

If you discover a security vulnerability in Conatus, please report it responsibly:

### Option 1: GitHub Security Advisory (Recommended)
Use GitHub's [Security Advisory](https://github.com/my-name-is-yu/Conatus/security/advisories) feature to report vulnerabilities privately. This allows coordinated disclosure before public announcement.
...
We consider the following as security issues in Conatus's scope:

- **Privilege escalation** — any ability to execute unauthorized actions with elevated permissions
- **Approval bypass** — circumventing Conatus's irreversible-action approval gates
- **State tampering** — unauthorized read/write access to `~/.conatus/` state files without proper protections
- **Plugin injection** — loading malicious code from `~/.conatus/plugins/` via path traversal or symlink attacks
...
The following are **not** Conatus's responsibility:
...
Conatus implements multiple layers of protection:
...
Conatus uses a failure-penalizing trust system (range `[-100, +100]`):
- Success reward: +3
- Failure penalty: −10
...
Conatus never executes these directly; it delegates to agents and verifies results.

### 4. Local State Isolation
- State stored in user's home directory: `~/.conatus/` (user-owned, not world-readable by default)
...
- Plugins load from `~/.conatus/plugins/` only
...
## Security Best Practices for Conatus Users

1. **Rotate API keys regularly** — treat `OPENAI_API_KEY` and `ANTHROPIC_API_KEY` like credentials
2. **Review goals before execution** — use `conatus goal list` and `conatus status` before running the core loop
3. **Audit plugin sources** — inspect `~/.conatus/plugins/` contents before trusting them
4. **Run Conatus in restricted environments** — use containers or VMs if you're automating high-risk tasks
5. **Monitor state changes** — check `~/.conatus/` periodically for unexpected modifications
6. **Use version pinning** — in `package.json`, pin Conatus to a specific version or narrow semver range

## Contact

- **GitHub**: [my-name-is-yu/Conatus](https://github.com/my-name-is-yu/Conatus)
