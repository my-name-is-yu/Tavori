...
MOTIVA="node dist/cli-runner.js"
...
for plugin in existing-plugin my-plugin shell-plugin; do
  if [ -d "$HOME/.motiva/plugins/$plugin" ]; then
    echo "Removing test artifact: $plugin" | tee -a "$LOG_DIR/summary.log"
    rm -rf "$HOME/.motiva/plugins/$plugin"
  fi
done
...
GOAL1_OUTPUT=$($MOTIVA goal add --title "zero-tsc-errors" --dim "tsc_error_count:min:0" 2>&1)
...
if [ -z "$GOAL1_ID" ]; then
  ...
  $MOTIVA run --goal "$GOAL1_ID" --yes --max-iterations 3 2>&1 | tee "$LOG_DIR/phase1.log"
...
PLUGIN_DIR="$HOME/.motiva/plugins/test-coverage-source"
mkdir -p "$PLUGIN_DIR"
...
GOAL2_OUTPUT=$($MOTIVA goal add --title "improve-test-coverage" --dim "test_coverage:min:80" --dim "test_count:min:4000" 2>&1)
...
if [ -z "$GOAL2_ID" ]; then
  ...
  $MOTIVA run --goal "$GOAL2_ID" --yes --max-iterations 2 2>&1 | tee "$LOG_DIR/phase2.log"
...
$MOTIVA plugin list 2>&1 | tee -a "$LOG_DIR/summary.log"
