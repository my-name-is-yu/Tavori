...
Motivaは**タスク発見エンジン**だ。ユーザーの長期的なゴール（「売上を2倍にしたい」「愛犬と幸せに暮らしたい」）を引き受け、現実世界を観測し、ゴールとのギャップから「次に何をすべきか」を発見し続ける。Motiva自身は何も実行しない。発見したタスクをエージェント（Claude Code等）に委譲し、結果を検証し、またループを回す。ゴールが達成されるまで、数日でも数年でも。
...
│                                                                       │
│  ┌────────────┐ ┌────────────┐ ┌────────────┐ ┌────────────────────┐ │
│  │ Claude Code │ │ Claude API │ │ カスタム    │ │ 人間               │ │
...
> **実装状況（2026-03-14時点）**: Stage 1-9完了 — 1266テスト通過、24テストファイル。
> 実装済みモジュール: StateManager, GapCalculator（Layer 0）、DriveSystem, TrustManager（Layer 1）、DriveScorer, ObservationEngine, SatisficingJudge, StallDetector（Layer 2）、LLMClient, EthicsGate, SessionManager, StrategyManager（WaitStrategy対応）, GoalNegotiator（Layer 3）、AdapterLayer, ClaudeCodeCLIAdapter, ClaudeAPIAdapter（Layer 0拡張）, TaskLifecycle（Layer 4）、ReportingEngine, CoreLoop（Layer 5）、CLIRunner（Layer 6）、KnowledgeManager, CapabilityDetector（Stage 8）、PortfolioManager（Stage 9）。TUI層（Stage 7-8, 11ファイル in src/tui/）。
> Zodスキーマ: 17ファイル（knowledge.ts, capability.ts, portfolio.ts を追加）
> 次ステップ: Stage 10+（memory-lifecycle, 高度な好奇心, マルチエージェント協調等）。詳細は docs/roadmap.md。
