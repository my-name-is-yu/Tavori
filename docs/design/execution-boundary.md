...
Motivaは以下のいずれかのシグナルが発生したとき、「現在の能力では対応できない」と判断する。
...
能力不足が確定したとき、Motivaは以下のフローで能力を調達する。
...
**適用ケース**: 能力不足が認証情報・権限・外部サービスへのアクセスに起因する場合。Motivaには要求できるが、自律的に取得できない種類の能力。
...
例:
  ...
  提案内容:
    - 連携するサービス: Slack
    - 取得できる能力: #motiva-alerts チャンネルへの通知送信
...
| 省略表現（不正確） | 正確な意味 |
|------------------|-----------|
| Motivaがコードを書いた | Motivaがコード実装エージェントにコード実装を指示し、結果を確認した |
...
| Motivaがツールを作った | Motivaがコード実装エージェントにツール作成を指示し、動作を検証した |
