...
> Motivaはユーザーに「聞かれる前に報告する」。適切なタイミングで、適切な粒度で、ゴール追求の状況を伝える。
...
Motivaはユーザーのゴールを自律的に追い続ける。しかし自律的であることは、黙って動くことではない。vision.md が定義するとおり、「Motivaの方から、適切なタイミングで、適切な粒度で報告する」ことが、自律パートナーとしての本質的な振る舞いだ。

レポーティングはMotivaの中核機能であり、オプションではない。ユーザーがMotivaを信頼できるかどうかは、透明性にかかっている（`trust-and-safety.md` 8 参照）。信頼は透明性によって支えられ、レポーティングは透明性の実装だ。

### レポーティングと実行境界

レポーティングの生成（内容の判断・構成）はMotivaが直接行う。これはMotivaの思考プロセスの一部であり、ゴール状態の分析・要約のためのLLM呼び出しに該当する。

レポートの配信（ファイル書き出し、通知送信）は委譲だ（`execution-boundary.md` 3 参照）。Motivaは「何を報告するか」を決め、「どう届けるか」は既存のシステムに委ねる。
...
**目的**: ゴール追求の全体像を定期的に伝える。ユーザーがMotivaの活動を俯瞰するための基盤情報だ。
...
| 次のアクション | 次の期間でMotivaが取り組む予定のタスク群 |
...
| 承認要求 | 不可逆アクションの実行前（`trust-and-safety.md` 4 参照） | アクション内容、トラスト/確信度、Motivaの評価、承認ボタン |
...
**目的**: Motivaが戦略を変更する際に、その判断根拠をユーザーに伝える。
...
Motivaの内部判断がトリガーとなる。外部からの入力ではなく、コアループ内の戦略選択ステップ（`mechanism.md` 2.3）で戦略変更が決定されたときに発火する。
...
`motiva run` 実行時に、未読のレポート・未処理の通知をコンソールに表示する。
...
[レポート] 日次サマリー (2026-03-10)
  ゴール「売上2x」: 進捗 42% → 45% (+3%)
  ゴール「愛犬の健康」: 進捗 88% (安定)
  詳細: ~/.motiva/reports/daily/2026-03-10.md

[通知] 戦略変更 (2026-03-10 14:30)
  ゴール「売上2x」: 「UI改善」→「サポート強化」にピボット
  詳細: ~/.motiva/reports/notifications/20260310-143022-strategy-change.md
```

### 5.2 Phase 2: 外部通知チャネル

Phase 2ではプッシュ通知を追加する。ユーザーがMotivaを確認しに来なくても、情報が届く。
...
| トリガー | 時刻ベース（`motiva run` 実行時に評価）、閾値ベース、イベントベース |
| 粒度制御 | 詳細度レベル（3段階）、通知クールダウン |
| LLM関与 | ナラティブ生成、戦略評価の言語化 |
| レポート保存 | `~/.motiva/reports/` にMarkdownファイルとして保存 |
| 未読管理 | ファイルベース（`motiva run` 時に未読レポートを表示） |

MVPの制約: レポートの配信はプル型（`motiva run` 実行時）のみ。リアルタイムのプッシュ通知はない。緊急アラートも次回の `motiva run` 実行時に表示される。これはMVPのプロセスモデル（CLI起動）の制約であり、レポーティング設計自体の制約ではない。
...
**レポートの過多 vs. 過少**: レポートが多すぎるとユーザーは無視し始める。少なすぎるとMotivaが何をしているかわからなくなる。デフォルト設定は「日次サマリー + 緊急時の即時通知」という保守的な構成にし、ユーザーが必要に応じて増減する設計にした。
...
**レポートの永続性**: レポートはMotivaの行動ログの一部でもある。「いつ、何を報告したか」は、Motivaの透明性と監査可能性の基盤だ。したがって、レポートファイルは削除ではなくアーカイブする。
