...
> Token cost is the primary barrier to Tavori adoption. A single loop iteration makes 3-7 LLM calls, meaning a goal with 3 dimensions and 10 iterations costs 50-70 API calls before any real work is done. This document defines three independent optimization pillars that together reduce LLM token consumption by an estimated 60-80% without degrading observation quality or loop correctness.
...
- **Context condensation / trajectory compression**: Tavori's LLM calls are stateless single-turn (no conversation history). Context condensation is unnecessary because there is no trajectory to compress.
